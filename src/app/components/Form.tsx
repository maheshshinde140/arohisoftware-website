    ...
    >
      ...
      <h1>{loading ? "Processing" : "Signup"}</h1>
        ...
        <h1>{loading ? "Processing" : " "}</h1>
          ...
          <button
            onClick={onSignup}
            className="flex w-full justify-center mb-4 rounded bg-indigo-600 hover:bg-indigo-500 px-4 py-2 text-sm font-semibold leading-6 text-white shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-600">{buttonDisabled ? " Signup" : "Loading..."}</button>
            {/* <Link href="/login">Visit login page</Link> */}
          </div>
          </form>
          <div className="relative my-4 h-px w-full bg-gray-600" />
          <form className="flex flex-col items-center space-y-3" action={async() => {
            await signIn("google")
          }}>
            <button
